...
export function ClientFinanceiroTab({
    ...
    const handleWhatsApp = async (tx: any) => {
        ...
        try {
            ...
            const msg =
                `Olíí ${clientName || ""}! ­ƒæï\n\n` +
                `Passando para lembrar sobre a cobraníºa:\n` +
                `­ƒôï *${tx.description}*\n` +
                `­ƒÆ░ *${formatCurrency(tx.amount)}*\n` +
                `­ƒôà Vencimento: ${vencimento}\n\n` +
            ...
            if (res.ok) {
                toast.success(`Cobraníºa enviada via WhatsApp para ${clientName || phone}!`);
            } else {
                toast.error("Erro ao enviar cobraníºa via WhatsApp");
            }
        } catch {
            toast.error("Erro ao enviar cobraníºa via WhatsApp");
        } finally {
    ...
    const handleEmail = () => {
        toast.info("Envio de e-mail de cobraníºa em breve.");
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-2xl border border-border/50">
                <h3 className="font-bold text-slate-800 dark:text-slate-200">Histórico de Transaíºàes</h3>
                <p className="text-[10px] text-muted-foreground uppercase font-medium">Gestão automatizada via Serviíºos</p>
            </div>
                        ...
                        <div key={tx.id} className="group flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-2xl border border-border/50 hover:border-indigo-500/30 transition-all gap-3">
                            <div className="flex items-center gap-4 min-w-0 flex-1">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-[10px] text-muted-foreground font-medium uppercase">{tx.category} ÔÇó {tx.dueDate ? formatDate(tx.dueDate) : "S/ data"}</p>
                                </div>
                            </div>

                            <div className="flex items-center gap-2 shrink-0">
                                {/* Botàes de cobraníºa ÔÇö só para receitas pendentes */}
