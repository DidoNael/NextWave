...
export default function WhatsAppConfigPage() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      toast.error("Erro ao carregar configuraíºàes");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Configuraíºàes salvas com sucesso!");
      } else {
        throw new Error();
      }
    } catch (error) {
      toast.error("Erro ao salvar configuraíºàes");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          >
            <ChevronLeft className="h-4 w-4" /> Voltar para Configuraíºàes
          </Link>
          ...
          <p className="text-muted-foreground">Configuraíºàes de integração com o módulo oficial do WhatsApp.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={onSave} disabled={saving} className="gap-2">
            ...
            {saving ? "Salvando..." : "Salvar Configuraíºàes"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-12">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <CardDescription>
                    Configure os dados de acesso à sua instíóncia da Evolution API.
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="token"
                    placeholder="Token da Instíóncia"
                    value={config.token}
        ...
        <div className="md:col-span-4 space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              {!config.isActive && (
                <div className="flex items-start gap-3 p-3 rounded-lg bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                  ...
                  <p className="text-xs">
                    Com o módulo inativo, as automaíºàes de WhatsApp e o Chat não funcionarão.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-md">Dica de Seguraníºa</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Nunca compartilhe sua <strong>API Key Global</strong>. Ela díí acesso total ao seu servidor Evolution API.
